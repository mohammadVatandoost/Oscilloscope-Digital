...
Knob2::Knob2( const QString &title, double min, double max, QWidget *parent ):
    QWidget( parent )
{
...
void Knob2::setValue( double value )
{
    d_knob->setValue( value );
}

double Knob2::value() const
{
    return d_knob->value();
}

void Knob2::setTheme( const QColor &color )
{
    d_knob->setPalette( color );
}

QColor Knob2::theme() const
{
    return d_knob->palette().color( QPalette::Window );
}

void Knob2::resizeEvent( QResizeEvent *event )
{
